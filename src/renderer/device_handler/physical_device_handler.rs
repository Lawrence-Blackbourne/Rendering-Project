use super::{QueueFamilyIndices, device_info_handler};
use crate::{renderer::RendererError, string_handler};
...
fn is_physical_device_suitable(
    ...
    let queue_family_indices =
        get_queue_family_indices(vulkan_instance, surface_instance, surface, device)?;
...
pub(super) fn get_queue_family_indices(
    ...
    let queue_families = unsafe {
        let num_queue_families =
            vulkan_instance.get_physical_device_queue_family_properties2_len(device);
    ...
    for (i, queue) in queue_families.iter().enumerate() {
        ...
        if found_queue_families.queue_family_indices[QueueFamilyIndices::GRAPHICS_QUEUE].is_none()
            && queue
...
fn check_device_extension_support(
    ...
    let available_extensions =
        unsafe { vulkan_instance.enumerate_device_extension_properties(device) }?;

    for needed_extension in super::DEVICE_EXTENSION_NAMES {
        let mut available = false;
        for available_extension in &available_extensions {
            let name =
                unsafe { string_handler::char_array_to_cstr(&available_extension.extension_name) };
...
fn get_device_display_info(
    ...
    device: vk::PhysicalDevice,
    surface: vk::SurfaceKHR,
) -> Result<device_info_handler::VulkanDisplayInfo, RendererError> {
...
mod tests {
    ...
    fn physical_device_is_suitable() {
        ...
        let surface_instance = tests::get_surface_instance(&vulkan_entry, &vulkan_instance);
        let physical_device =
            get_physical_device(&vulkan_instance, &surface_instance, surface).unwrap();
